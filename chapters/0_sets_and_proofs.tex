\chapter{Sets and Proofs}\label{chap:fundamentals}
...
\begin{defn}
A mapping $ f:A\to B $ is called \defnem{injective} if for all $ a,\tilde{a}\in A $, if $ f(a)=f(\tilde{a}) $, then $ a=\tilde{a} $; in other words, $ f(a) $ is unique. $ f $ is called \defnem{surjective} if for all $ b\in B $, there exists an $ a\in A $ such that $ f(a)=b $. A mapping that is both injective and surjective is called \defnem{bijective}.
\end{defn}

\begin{defn}
Let $ f:A\to B $ be a mapping. The set $ \{f(a)\mid a\in A\} $ is called the \defnem{range} of $ f $.
\end{defn}

\begin{defn}\label{defn:binop}
Let $ A $ and $ B $ be two non-empty sets. A mapping $ \odot:B\times A\to A $, $ (x,y)\mapsto x\odot y $, is called a \defnem{binary operation}. If $ A=B $, i.e. we have $ \odot:A\times A\to A $, we say $ \odot $ is a binary operation on $ A $.
\end{defn}
